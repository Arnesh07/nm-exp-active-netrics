...
def build_parser():
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='Store youtube data to influxDB'
    )
...
def upload(upload_results, measurements):
    ...
    print(server, port, username, password, db, installid)

    if server is None:
        log.error(
            "Unable to insert data (influxdb), missing .env or INFLUXDB_* not set.")
    ...
    creds = InfluxDBClient(host=server, port=port, username=username,
                           password=password, database=db, ssl=False, verify_ssl=False)
...
#       raise Exception('User: {0}'.format(userid))
    ...
    print(
        "ERROR: run this command as using unprivileged user 'netrics' or root, message:({0})".format(error))
    sys.exit(1)
...
connectivity_status = [
    stat for stat in connectivity_status if stat is not None]
...
if not connectivity_failure:
    ...
    output['last_mile_rtt'] = test.last_mile_latency(
        'last_mile_rtt', args.last_mile_rtt)

    """ Measure DNS latency """
    output['dns_latency'] = test.dns_latency('dns_latency', args.dns)

    """ Count hops to target website """
    output['hops_to_target'] = test.hops_to_target(
        'hops_to_target', args.target)
        
    """ Store youtube data for given url """
    output['youtube'] = test.store_youtube_data(
        'youtube', args.yt, args.id, args.link, args.headless, args.mode)
    ...
    if args.speed:
        output['ookla'], output['ndt7'] = test.speed(
            'ookla', 'ndt7', args.limit_consumption)
...
output['connected_devices_arp'] = test.connected_devices_arp(
    'connected_devices_arp', args.ndev)

""" Measure consumption using tshark. """
output['tshark_eth_consumption'] = test.tshark_eth_consumption(
    'tshark_eth_consumption', args.tshark)
...
if args.reset_consumption:
    ...
    test.results['total_bytes_consumed'] = test.consumption_update(
        test.results['total_bytes_consumed'])
...
nma.save_json(test.results, 'netrics_results', timenow,
              extended=nma.conf['extended'] if 'extended' in nma.conf.keys(
              ) else None,
              annotation=args.annotate)
