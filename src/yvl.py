...
class Youtube_Video_Loader:
    ...
    def __init__(self, _id, is_headless):

        self.t_initialize = time.time()

        # how often to look at stats for nerds box (seconds)
        self.pull_frequency = .5

        # how long before the end of the video to stop (seconds)
        self.early_stop = 10
        ...
        if(is_headless == 'false'):
            ...
            chrome_options.add_argument("--window-size=2000,3555")
        ...
        self.driver = webdriver.Chrome(
            "/usr/bin/chromedriver", chrome_options=chrome_options, desired_capabilities=caps)
        ...
        if (is_headless == 'true'):
            ...
            self.log_prefix = "youtube_stats_log_headless_{}-".format(self._id)

        if (is_headless == 'false'):

            self.log_prefix = "youtube_stats_log_head_{}-".format(self._id)
    ...
    def save_screenshot(self, img_name):
        ...
        self.driver.save_screenshot(os.path.join(
            self.error_report_dir, "youtube_" + img_name))
    ...
    def shutdown(self):
        ...
        for link in self.video_statistics:
            ...
            video_hash = re.search(
                "youtube\.com\/watch\?v=(.+)", link).group(1)
            ...
            with open(fn + "-stats.csv", 'w') as f:
                ...
                csvw = csv.DictWriter(
                    f, list(self.video_statistics[link]["stats"][0].keys()))

                csvw.writeheader()

                [csvw.writerow(row)
                 for row in self.video_statistics[link]["stats"]]

            pickle.dump(self.video_statistics[link]["metadata"], open(
                fn + "-metadata.pkl", 'wb'))
    ...
    def get_bitrate_data(self, link):
        ...
        try:
            ...
            available_formats = check_output(
                "youtube-dl {} --list-formats".format(link), shell=True).decode('utf-8')
                ...
                try:
                    ...
                    try:
                        ...
                        if extension not in ["mp4", "webm"]:
                            ...
                            raise ValueError(
                                "Unprepared to handle extension: {}".format(extension))
            ...
            for resolution in resolution_to_format:
                ...
                call("youtube-dl -o tmp.{} -f {} {}".format(fmt,
                     code, link), shell=True)

                # get the bitrates for this video

                raw_output = check_output(
                    "ffmpeg_bitrate_stats -s video -of json tmp.{}".format(fmt), shell=True)
    ...
    def run(self, link):
        ...
        try:  # lots of things can go wrong in this loop TODO - report errors
            ...
            # a common error is that the page takes a little long to load, and it cant find the player
            time.sleep(3)
            ...
            while True:
                ...
                try:
                    ...
                    player = self.driver.find_element_by_css_selector(
                ...
                if i == max_n_tries:
                    ...
                    print(
                        "Max number of tries hit trying to get the player-container-inner. Exiting.")

                    self.save_screenshot(
                        "player_container_unable_{}.png".format(self._id))
            ...
            self.driver.find_element_by_css_selector(
                'div:nth-child(7) > div.ytp-menuitem-label').click()
            ...
            if not stats_for_nerds_i:
                ...
                self.save_screenshot(
                    "no_stats_for_nerds_{}.png".format(self._id))
            ...
            while True:
                ...
                video_length = self.driver.find_element_by_css_selector(
                    '.ytp-time-duration').text.split(":")

                try:

                    video_length = 60 * \
                        int(video_length[0]) + int(video_length[1])
                    ...
                    # bring up the video length box again
                    ...
                    if i == max_n_tries:
                        ...
                        raise ValueError(
                            "Couldn't find video length: {}".format(video_length))

                    i += 1

            tick = time.time()

            self.video_statistics[link]["metadata"]["start_wait"] = tick - \
                self.t_initialize
            ...
            find_str = '//*[@id="movie_player"]/div[{}]'.format(
                stats_for_nerds_i)
            ...
            while not stop:
                ...
                video_stats_re = re.search(
                    "\[x\]Video ID (.+)Viewport \/ Frames(.+) Optimal Res(.+)Volume(.+)Codecs(.+)Connection Speed(.+) KbpsNetwork Activity(.+) KBBuffer Health(.+) sLive LatencyLive ModePlayback CategoriesMystery Texts:(.+) t\:(.+) b", video_stats_text)
                ...
                try:
                    ...
                    # 4 -> paused, 5 -> paused&out of buffer, 8 -> playing, 9 -> rebuffering
                    state = int(mystery_text)
                ...
                if last_playback_progress is not None and buffer_health > .5:
                    ...
                    if no_pb_progress_ctr > self.max_pb_ctr_allowance:
                        ...
                        self.save_screenshot(
                            "not_progressing_{}_youtube.png".format(self._id))
                ...
                time.sleep(np.maximum(self.pull_frequency -
                           (time.time() - t_calls), .001))
...
def initializeAndRun(id, headless, link, mode):
    ...
    yvl = Youtube_Video_Loader(id, headless)
...
def main():
    ...
    parser.add_argument('--headless', action='store')

    args = parser.parse_args()

    initializeAndRun(args.id, args.headless, args.link, args.mode)
